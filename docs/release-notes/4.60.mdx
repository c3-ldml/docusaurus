...
import nluMigration from '@site/docs/_includes/release-notes/_nlu-migration.mdx';
import twilioAutopilotDeprecation from '@site/docs/_includes/release-notes/_twilio-autopilot-deprecation.mdx';
...
- Removed the feature flag hiding the [Skills](../live-agent/settings/skills.mdx) page
...
- Added [Plug-and-Play Widgets](../ai-copilot/plug-and-play-widgets.mdx) to improve the Agent Assist Workspace Experience with Next Action Assist, Sentiment Assist, Transcript Assist and Identity Assist

#### Improvements

- Added the [Stream to Output](../ai/build/node-reference/service/llm-prompt.mdx) option in the Storage & Streaming Options section of the LLM Prompt Node
- Added the [Bandwidth](../ai/deploy/endpoint-reference/bandwidth.mdx) Endpoint
- Added the [Error Handling](../ai/build/node-reference/other-nodes/search-extract-output.mdx) section to the Search Extract Output Node
- Added the opportunity to configure whether [extensive data payloads](../ai/deploy/endpoints/data-protection-and-analytics.mdx#store-extensive-data-payloads-in-analytics) will be stored in Analytics
...
overview.mdx#limitations) from 500 to 1000
- Improved by making the Snapshot up and download logic more reliable
- Added the [No Answer](../voice-gateway/references/events/NO_ANSWER.mdx) Voice Gateway event
- Added [Call Event](../ai/deploy/endpoint-reference/voice-gateway.mdx#call-events) Handling as a section to the VG Endpoint, enabling the addition of actions to any Call event
...
- Added support for [skill and language-based routing](../live-agent/conversation/conversation-routing/automatic-mode.mdx#additional-automatic-assignment-parameters)
...
- Implemented the ability to include [multiple STT/TTS](../voice-gateway/webapp/applications.mdx#add-additional-tts-and-stt-vendor) speech credentials and set up a fallback option for cases when the default one fails
- Added [Microsoft Azure](../voice-gateway/webapp/accounts.mdx#call-recording-configuration) as a storage option for call recordings
- Added [Google Cloud](../voice-gateway/webapp/accounts.mdx#call-recording-configuration) as a storage option for call recordings
- Added the [Delete Recorded File](../voice-gateway/webapp/recent-calls.mdx#call-recordings) button to delete the recording from the remote storage
...
For on-premises Cognigy v4.60 or later, migrating from NLU V1 to NLU V2 is necessary. We have enhanced NLP services for scalability, security, and reliability by dividing them into smaller, independently scalable components. To migrate to the new version, refer to the [Cognigy NLU: from V1 to V2](../ai/installation/migration/from-nlu-v1-to-v2-migration.mdx) guide.

#### Version Compatibility Matrix

The version compatibility matrix was updated for the following Cognigy products:

- [Cognigy.AI and Insights](../ai/installation/version-compatibility-matrix.mdx)
- [Live Agent](../live-agent/installation/deployment/version-compatibility-matrix.mdx)
- [Agent Assist Workspace](../ai-copilot/installation/version-compatibility-matrix.mdx)
