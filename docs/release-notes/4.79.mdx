...
- Added the [Advanced Parser](../ai/empower/knowledge-ai/knowledge-source/text-extraction/advanced-parser.mdx) for Knowledge AI. This Parser is activated by default if configured for your installation 
- Added support for PPTX format in both the [Basic](../ai/empower/knowledge-ai/knowledge-source/text-extraction/basic-parser.mdx) and [Advanced](../ai/empower/knowledge-ai/knowledge-source/text-extraction/azure-ai.mdx) Parsers
- Added the API [`Post /v2.0/projects/{projectId}/nlu/scores`](../ai/empower/nlu/intents/intent-analyzer.mdx#testing-nlu-scores) request for matching and scoring sentences with NLU Intents
...
- The feature [Atmosphere Sounds](../ai/build/node-reference/voice/voice-gateway/parameter-details.mdx) adds the possibility to insert additional audio tracks into the conversation. One common usage is to add background sounds of keyboards clicking to simulate an office environment.
...
- Added the capability to generate Adaptive Cards via Generative AI in the [Copilot: Adaptive Card Tile Node](../ai/build/node-reference/ai-copilot/set-adaptive-card-tile.mdx)
