...
import handoverProvidersDeprecation from '@site/docs/_includes/release-notes/_handover-providers-deprecation.mdx';
...
:::warning[Upcoming Feature Deprecations and Removals]
  ...
  - On April 1, 2025, [Insights Beta API](https://api-trial.cognigy.ai/openapi#get-/insights/beta/messages/report/-queryHash-) will be deprecated. Instead, use the [Cognigy.AI OData endpoint](../ai/analyze/odata.mdx).
  ...
  - On September 9, 2025, OData v2.3 will be deprecated. Before this date, ensure a smooth transition by switching to the [Cognigy.AI OData v2.4](../ai/analyze/odata.mdx#supported-versions).
...
Introduced a new [interface for handover providers](../ai/escalate/migration.mdx).
...
Introduced [Global LLMs](../ai/administer/access/admin-center/global-resources/overview.mdx) in the Admin Center, allowing assignment to any Project within your organization and streamlining LLM management across Projects.

#### Improvements

- Renamed the option from Log Token Count to Show Token Count to more accurately reflect its functionality. This change affects the following Nodes: [Search Extract Output](../ai/build/node-reference/other-nodes/search-extract-output.mdx), [LLM Prompt](../ai/build/node-reference/service/llm-prompt.mdx), [LLM Entity Extract](../ai/build/node-reference/other-nodes/llm-entity-extract.mdx), and [AI Agent](../ai/build/node-reference/ai/ai-agent.mdx)
- Added a new LLM type, [OpenAI Compatible](../ai/empower/llms/providers/openai-compatible.mdx), which allows users to integrate any provider and model with an API that adheres to OpenAI's API standards
- Added the Exclude from Transcript option to the Advanced section in [Say](../ai/build/node-reference/basic/say.mdx#advanced) and [Question](../ai/build/node-reference/basic/question.mdx#exclude-from-transcript) Nodes to hide an output message from AI Agents
- Added support for [Claude 3.7 Sonnet](../ai/empower/llms/model-support-by-feature.mdx) by Anthropic
...
Added Deepgram as a voice provider to the [Voice Preview](../ai/test/voice-preview.mdx#set-up-a-voice-preview-provider) feature, enabling you to test your voice AI Agent using your Deepgram credentials in the Interaction Panel.

#### Improvements

- Added the `notifyURL` parameter to the list of parameters for creating an [Outbound Call API request](../voice-gateway/creating-outbound-calls.mdx#basic-configuration-request), enabling status updates for the call
...
- Added the capability to configure translations for Webchat UI elements in the [Webchat Custom Settings](../webchat/v3/configuration.mdx#webchat-custom-settings) of the Webchat v3 Endpoint
- Added the option to customize the [Start new conversation button text](../webchat/v3/configuration.mdx#previous-conversations) in the Webchat v3 Endpoint
...
- [Cognigy Live Agent](../live-agent/installation/deployment/version-compatibility-matrix.mdx)
