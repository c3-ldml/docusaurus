...
import weaviateUpgrade from '@site/docs/_includes/release-notes/_weaviate-upgrade.mdx';
import textDavinci003Sunset from '@site/docs/_includes/release-notes/_text-davinci-003-sunset.mdx';
...
- Added the [Copilot: Knowledge Tile](../ai/build/node-reference/ai-copilot/knowledge-tile.mdx) widget for AI Copilot
- Added the [Reprompt Methods](../ai/build/node-reference/basic/question.mdx#reprompt-methods) to the Reprompt options in the Question Node
...
- Added the button to forward the next best action content from the AI Copilot widget to the Live Agent reply section. The improvement is in the [Copilot: Knowledge Tile](../ai/build/node-reference/ai-copilot/knowledge-tile.mdx#configuration) and [Copilot: Next Action Nodes](../ai/build/node-reference/ai-copilot/next-action-tile.mdx#use-case)
- Added the correct userId and sessionId for transcriptions to the [Voice Copilot Endpoint](../ai/deploy/endpoint-reference/voice-copilot.mdx)
...
jobSyncKnowledgeData:
  ...
  extraEnvVars:
    - name: JOB_ARGUMENTS
      value: "eyJhY3Rpb24iOiJyZXN0b3JlVmVjdG9yREIiLCJv.mdxhbmlzYXRpb25JZHMiOltdLCJwcm9qZWN0SWRzIjpbXSwic3RvcmVJZHMiOltdLCJ0YXNrSWQiOiIiLCJ0cmFjZUlkIjoidHJhY2UtcmVzdG9yZVZlY3RvckRCIiwidXBkYXRlQmF0Y2hTaXplIjo0MH0="
```
