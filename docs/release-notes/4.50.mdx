...
- Added the [8x8 Endpoint](../ai/deploy/endpoint-reference/8x8.mdx) and the [8x8 handover provider](../ai/escalate/handover-reference/8x8.mdx)
- Added a progress indicator for snapshot download 
- Improved by sending an email out once a user is added to a Cognigy project via [Access control](../ai/administer/access/admin-center/access-control.mdx) or [Members](../ai/administer/access/members.mdx)
- Added the default favicon to the xApp page, changing the default page title from "App Shell Page" to "xApp" and allowing users to customize the favicon and page title in the [xApp: Init Session Node](../ai/build/node-reference/xApp/init-xApp-session.mdx)
- Disabled snapshot creation buttons when a limit is exceeded

#### Bug Fixes

- Fixed the issue of [limiting the number of characters](../ai/build/node-reference/basic/code/overview.mdx#limitations) allowed in the Code Node editor
...
- Added the [Transcribe section](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) with Speech Recognizer options and Audio Stream selection to the Voice Gateway Transfer Node
