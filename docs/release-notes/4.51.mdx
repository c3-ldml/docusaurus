...
- Added [xApps](../xApps/overview.mdx) in Beta. Cognigy xApps are mobile first, micro web applications that empower enterprises to seamlessly connect all channels and enable personalized, multimodal self-service solutions uniting text, visuals, and voice. This feature is available to all Cognigy users
- Added [Agent Assist Workspace](../ai-copilot/overview.mdx) in Beta. Cognigy Agent Assist Workspace offers a variety of advanced features that empower agents to provide faster and more accurate customer support. The workspace is fully customizable with widgets that display the most relevant information for any chat and voice use case
- Added the handling of [Voice Gateway Call Events](../ai/deploy/endpoint-reference/voice-gateway.mdx#call-events) via the [Lookup](../ai/build/node-reference/logic/lookup.mdx) Node

#### Improvements

- Added [Functions](../ai/empower/nlu/text-cleaner.mdx) and [Nodes](../ai/build/node-reference/ai/clean-text.mdx) to automatically parse utterances, including spelling (alpha echo lima tango) to the correct strings
- Added `uniqueIntentId` and `uniqueIntentFlowId` to the input object
- Shortened the xApps URL for improved usability.
- Moved the [Page Title](../ai/build/node-reference/xApp/init-xApp-session.mdx) field to the Style Customization section in the xApp: Init Session Node
...
- Renamed the Live Agent Escalations chart to [Handover in Time](../insights/reports/engagement.mdx#handovers-in-time) chart and updated its descriptions in the Engagement dashboard
...
- Embedded the [AI Copilot workspace](../live-agent/assistants/ai-copilot.mdx) functionality into Live Agent. This feature is in Beta
...
- Added the [Answering Machine Detection](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) to the Voice Gateway Transfer Node
