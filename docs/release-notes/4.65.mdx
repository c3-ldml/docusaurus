...
import aawToHelmChartMigration from '@site/docs/_includes/release-notes/_aaw-to-helm-chart-migration.mdx';
import singleReplicaMigration from '@site/docs/_includes/release-notes/_single-replica-migration.mdx';
...
- Fixed the issue where a REST Outdial with .mdx actionHook and a separate `gather` verb failed to capture speech
...
- Added [Aleph Alpha text generation model support](../ai/empower/llms/model-support-by-feature.mdx) for LLM Prompt Node & LLM-powered Answer Extraction
- Added the [Custom model](../ai/empower/llms/overview.mdx#custom-models) parameter for the OpenAI and Aleph Alpha LLM providers
- Implemented a [limit for the tasks](../ai/build/projects.mdx) displayed in the task menu to 100
- Renamed the label for Voice Gateway setup on the Organization Details page
- Added the [Smart Formatting and Endpointing](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) options for the Deepgram STT provider
...
- Product Launch: [AI Copilot](../ai-copilot/overview.mdx)

#### Improvements

- Improved by reducing the number of mandatory parameters of the [AI Copilot URL](../ai-copilot/configuration.mdx#url)
...
make sure to perform the [Redis: Single Replica to Redis HA Migration](../ai/installation/migration/single-replica-redis-to-redis-ha-migration.mdx) guide before upgrading to Cognigy.AI v4.65.
...
Upgrade to Cognigy.AI 4.65 and new Helm chart for continued support. Follow this [migration guide](../ai-copilot/installation/migration/agent-assist-to-cognigy-ai-helm-chart-migration.mdx) for details.
