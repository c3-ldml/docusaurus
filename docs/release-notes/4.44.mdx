...
- Added the [Adaptive Card](../ai/build/node-reference/basic/say.mdx) type to the Output Type list for the Say and Question Nodes
- Added [Generative AI](../ai/empower/generative-ai.mdx) (available only for Beta program participants)
...
- Added the opportunity to create [multiple roles](../live-agent/roles.mdx) for users
...
- Upgraded PostgreSQL and Redis dependencies to the latest version in the Helm Chart (does not apply to external instances). For more information, see [migration guide](../live-agent/installation/migration/helm-dependencies.mdx)
...
- Added the [Dial Transfer Type](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) to the Voice Gateway Transfer Node. If you want to use this update, add a new Voice Gateway Transfer Node in the Flow Editor and manually transfer the required settings from the old Node.
