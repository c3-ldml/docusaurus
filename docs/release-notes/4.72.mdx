...
import aiCopilotDnsNames from '@site/docs/_includes/release-notes/_ai-copilot-dns-names.mdx';
import deprecatedAzureModels from '@site/docs/_includes/ai/_deprecated-azure-models.mdx';
import handoversBrokenFourSevenOneFourSevenTwo from '@site/docs/_includes/release-notes/_handovers-broken-4.71-4.72.mdx';

# 4.72

<handoversBrokenFourSevenOneFourSevenTwo />
...
- The [ElevenLabs Streaming TTS](../voice-gateway/webapp/streaming.mdx) feature is not available. We have switched your settings to traditional TTS
...
- Added [Webchat v3](../webchat/v3/overview.mdx) as a preview feature. This is a new improved version of Webchat with updated design and various settings for widget customization. We encourage you to try it out and provide us with feedback.

#### Improvements

- Implemented the [LLM Entity Extract](../ai/build/node-reference/other-nodes/llm-entity-extract.mdx) Node and added the [LLM Entity Extraction Options](../ai/build/node-reference/basic/question.mdx#) to the Question Node
- Added the capability to activate the Copy button within the [Copilot: Next Action Tile](../ai/build/node-reference/ai-copilot/next-action-tile.mdx) and [Copilot: Knowledge Tile Nodes](../ai/build/node-reference/ai-copilot/next-action-tile.mdx) for all handover providers except Live Agent. For Salesforce, ensure to update your Salesforce integrations using [this guide](https://github.com/Cognigy/salesforce-integrations?tab=readme-ov-file#update)
- Extended the functionality of the AI Copilot: Next Action Tile Node to include the [Knowledge Assist](../ai/build/node-reference/ai-copilot/next-action-tile.mdx#settings) options
...
- Enabled the [AI Copilot Embedding URL](../ai/escalate/handover-reference/genesys-cloud-open-messaging.mdx#embedded) for the Genesys Open Message handover provider
...
- Added the capability to [select a handover provider](../ai/deploy/endpoint-reference/voice-gateway.mdx#handover-settings) for the Voice Gateway Endpoint. After handover initiation, a `gather` verb is sent to collect transcripts and forward them to the human agent 
...
- Added the UI element to [change the size of the conversation chat interface](../live-agent/conversation/other-operations.mdx#expand-or-collapse-the-chat-area)
