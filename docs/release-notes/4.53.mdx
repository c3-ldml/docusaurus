...
import k8cSupport from '@site/docs/_includes/release-notes/_k8c-support.mdx';
import mongoDbMigration from '@site/docs/_includes/release-notes/_mongo-db-migration.mdx';
...
- Added the [Inactivity Detection and Handling](../ai/escalate/user-inactivity-detection.mdx) feature. This feature detects end-user inactivity during handovers, enabling auto-closing, automated responses, and triggering actions

#### Improvements

- Added support for [Anthropic's LLMs](../ai/empower/llms/model-support-by-feature.mdx), including `claude` and `claude-instant`
- Added the capability to [test an LLM connection](../ai/empower/llms/providers/all-providers.mdx) 
...
- Improved the [Microsoft Azure OpenAI Connection type](../ai/empower/llms/providers/microsoft-azure-openai.mdx#add-a-model) in the LLM settings
- Improved by updating demo webchat to v2.54.0
- Added the [Mask IP Address](../ai/deploy/endpoints/data-protection-and-analytics.mdx#mask-ip-address) toggle to the Data Protection Endpoint editor settings. This setting is available for most of the Endpoints
...
- Added the ability to create [scoped canned responses](../live-agent/settings/canned-responses.mdx#create-a-canned-response) at the personal, team, and global levels

#### Improvements

- Improved by saving team names with the initial formatting
- Improved by allowing conversations to be unassigned from busy agents. To change this behavior, use the [Assign conversation to a busy agent](../live-agent/settings/account-settings.mdx#auto-assign-conversations-to-a-busy-agent) setting
...
- Added the [Call Tracing](../voice-gateway/webapp/recent-calls.mdx) feature to the Recent calls section in the Self-Service Portal
...
- The Redis Sentinel support has been added. For more information, please refer to the [Migration guide for setting up Redis Sentinel in Live Agent](../live-agent/installation/migration/redis-sentinel.mdx) article.
