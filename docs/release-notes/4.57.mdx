...
- Added the following [Voice Gateway Events](../voice-gateway/references/events/overview.mdx) that can be managed within a Flow:
    - `TRANSFER_REFER_SUCCESS`
    - `TRANSFER_REFER_ERROR`
    - `TRANSFER_DIAL_SUCCESS`
    - `TRANSFER_DIAL_ERROR`
    - `USER_BUSY`
- Added the [Set Copilot Grid](../ai/build/node-reference/ai-copilot/set-grid.mdx) Node for changing the AI Copilot workspace layout in the Copilot flow dynamically
...
- Implemented the [Consider Conversation Count](../live-agent/conversation/conversation-routing/automatic-mode.mdx) setting as a new strategy to identify the most suitable agent for the assignment
- Extended the [Audit Logs](../live-agent/settings/audit-logs.mdx) by showing detailed information on CRUD operations for Teams, Inboxes, and Automation Rules
- Added the capability for the [Labels](../live-agent/settings/labels.mdx) to be scoped for Inbox, Team, or Account
...
- Added the [Outbound Call Routing](../voice-gateway/webapp/outbound-call-routing.mdx) feature that allows you to select a carrier (SIP trunk) for outbound calls and configure specific rules for call routing
