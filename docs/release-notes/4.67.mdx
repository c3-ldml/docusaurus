...
- Added the [Generic Settings](../ai/deploy/endpoint-reference/voice-gateway.mdx) section to the Voice Gateway Endpoint
...
- Added [ElevenLabs](../voice-gateway/references/tts-and-stt-vendors.mdx) as a Text-to-Speech (TTS) provider
- Improved the Voice Gateway Endpoint by adding a setting to trim `input.data` for better transcripts
- Added support to import the [secondary locale of the package](../ai/build/packages.mdx) as the primary locale of the agent
...
- Added the capability for human agents to create draft messages. If a [reply](../live-agent/conversation/send-reply.mdx#send-a-reply-to-a-user) or a [private](../live-agent/conversation/send-reply.mdx#send-a-reply-to-another-human-agent) message is composed but not sent immediately, the message will be saved for 24 hours

#### Improvements

- Renamed Live Agent Assist Bot to [Copilot Bot](../live-agent/assistants/ai-copilot-bot.mdx)
