...
- Added the [Large Language Model (LLM)](../ai/empower/llms/model-support-by-feature.mdx) as a new resource for the project, including support for the `gpt-3.5-turbo` model and configuration for various Generative AI use cases in the Agent settings
...
- Added the settings for both [maintenance](https://github.com/Cognigy/WebchatWidget/blob/master/docs/embedding.mdx#maintenance) and [out-of-business-hours](https://github.com/Cognigy/WebchatWidget/blob/master/docs/embedding.mdx#business-hours) modes to the Webchat Endpoint
...
- Extended the [Answering Machine Detection](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) section in the Voice Gateway Transfer Node for the Dial type
- Added the [Genesys section](../ai/escalate/handover-reference/genesys-cloud-guest-chat.mdx#configure-handover-settings) to the Handover to Agent Node to extend it with 3 new fields and a JSON field to send Custom attributes when doing the handover
- Applied a new Continuous Counting Method to count conversations based on [24-hour time window](../ai/administer/billing.mdx#common-scenarios)

#### Bug Fixes

- Fixed the issue where the [maximum slot limit](../ai/empower/nlu/slots-and-lexicons/lexicons.mdx#restrictions) allowed for the lexicon was not considered during lexicon CSV upload. To support backward compatibility, we made this limit configurable
...
- Added the opportunity to [import canned responses via CSV](../live-agent/settings/canned-responses.mdx#import-canned-responses)
