...
- Added the [Generic Mute Speech Input](../ai/build/node-reference/voice/generic/mute-speech-input.mdx) and [Voice Gateway Mute Speech Input](../ai/build/node-reference/voice/voice-gateway/mute-speech-input.mdx) Nodes

#### Improvements

- Added a toggle to turn off the [TTS audio cache](../ai/build/node-reference/voice/voice-gateway/parameter-details.mdx) in the Synthesizer (TTS) section of both the Voice Gateway Set Session Config Node and the Voice Gateway channel responses with Set Activity Parameters for the Question, Optional Question, Say, and Play Nodes
...
- Added the [Audit Logs](../live-agent/settings/audit-logs.mdx) section where the activity of an account can be checked in the form of events
...
- Added the [indicator for tiles](../ai-copilot/configuration.mdx#tile-id) that are not presented in the AI Copilot workspace layout
