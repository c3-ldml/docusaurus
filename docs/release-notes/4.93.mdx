...
:::warning[Upcoming Feature Deprecations and Removals]
  ...
  - [AudioCodes Nodes](../ai/build/node-reference/voice/audiocodes/overview.mdx) and [Generic Voice Nodes](../ai/build/node-reference/voice/generic/overview.mdx) will be deprecated at the end of February 2025. After this time, you won't be able to create new Nodes, but you can still edit or clone existing ones.
...
With this release, we introduce support for the powerful [`amazon.titan-embed-text-v2:0`](../ai/empower/llms/model-support-by-feature.mdx) embedding model by Amazon Bedrock. This model is now available for Cognigy Knowledge AI and hidden behind the `FEATURE_ENABLE_AWS_BEDROCK_EMBEDDING_LLM_WHITELIST` feature flag.
...
- Added the [Watermark URL](../webchat/v3/configuration.mdx#watermark) parameter to the Watermark section in the Webchat v3 Endpoint
- Moved the [Webchat Plugins](../webchat/v3/configuration.mdx#webchat-plugins) settings to the Demo Webchat Settings section in the Webchat v3 Endpoint
...
- Fixed the issue with the [File Upload](../live-agent/conversation/send-reply.mdx#request-a-file-upload) plugin where files sent by the end user were uploaded but displayed as Unknown File in the conversation chat
...
- Added the [Goals Performance (in numbers)](../insights/reports/goals.mdx) chart to the Goals dashboard
- Shortened the [default date range](../insights/global-filters.mdx) in the Insights global filters from `30 days` to `7 days`
