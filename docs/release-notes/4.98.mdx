...
:::warning[Deprecations in 4.98]
  ...
  - [Insights Beta API](https://api-trial.cognigy.ai/openapi#get-/insights/beta/messages/report/-queryHash-). Instead, use the [Cognigy.AI OData endpoint](../ai/analyze/odata.mdx).
...
Introduced a [new handover provider for Salesforce MIAW](../ai/escalate/handover-reference/salesforce-miaw.mdx). With the Salesforce MIAW handover connector, you can integrate Cognigy.AI with Salesforce Messaging for In-App and Web, enabling end users to connect with human agents working in a contact center that uses Salesforce.

#### Improvements

- Improved AI Agent voice capabilities in the [AI Agent creation wizard](../ai/empower/agentic-ai/manage-ai-agents.mdx#create-from-scratch) and [AI Agent Node](../ai/build/node-reference/ai/ai-agent.mdx#voice). Now, you can add labels for TTS vendors and disable the audio cache
- Removed the beta flag from the [Yes/No Intents](../ai/empower/nlu/intents/yes-no-intents.mdx) feature
...
- Added the `notifyURL` parameter for additional cases, enabling proper reporting of `REFER` and `DIAL` success and failure statuses via the `call:status` [webhook event](../voice-gateway/creating-outbound-calls.mdx#basic-configuration-request)
...
- Renamed Goals to Tasks in the [Contact Profile](../ai/analyze/contact-profiles.mdx) to standardize naming conventions across Cognigy.AI
- Replaced the `goals` property in the [Profile object](../ai/test/interaction-panel/profile.mdx) with `tasks`
