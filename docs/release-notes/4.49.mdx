...
- Added the OAuth2 authentication method to the [Send SMTP Email](../ai/build/node-reference/data/send-smtp-email.mdx) and [Email Notification](../ai/build/node-reference/data/email-notification.mdx#authentication) Nodes
...
- Made the [security improvement](../ai/administer/user-menu/my-profile.mdx#api-keys) by displaying API keys only once during creation and storing their hashes
...
- Added the Wait for Input option in the [xApp: Show Adaptive Card](../ai/build/node-reference/xApp/set-AdaptiveCard-xApp-state.mdx) and [xApp: Show HTML Nodes](../ai/build/node-reference/xApp/set-html-xApp-state.mdx)
...
- Added a [search bar](../insights/explorers/transcript.mdx) in the step filter in Transcript Explorer
- Improved Transcript Explorer by changing the JSON messages to expandable messages
- Added four [indicator chars with ongoing statuses](../insights/reports/live-agent.mdx#opened) to the Live Agent dashboard
- Replaced the Conversation Summary chart with the [Session End Reason](../insights/reports/live-agent.mdx#handover-end-reason) chart
- Added [LiveAgentEscalations](../ai/analyze/odata.mdx#liveagentescalations) to Cognigy.AI OData Collections
