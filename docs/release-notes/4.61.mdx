...
import nodejs16EndOfSupport from '@site/docs/_includes/release-notes/_nodejs-16-end-of-support.mdx';
...
- Add the [Call Failover](../ai/deploy/endpoint-reference/voice-gateway.mdx) settings to the Voice Gateway Endpoint to enable call transfers in case of Flow runtime errors

#### Improvements

- Added the toggle to the [Mute Speech Input Node](../ai/build/node-reference/voice/voice-gateway/mute-speech-input.mdx) to control whether a DTMF input collection is muted or not
- Added the [Transfer Refer Error](../voice-gateway/references/events/TRANSFER_REFER_ERROR.mdx) and [Transfer Dial Error](../voice-gateway/references/events/TRANSFER_DIAL_ERROR.mdx) call events to the Voice Gateway Endpoint. This enables the option to initiate a new outgoing call or refer to the current call in case of a previous transfer error
- Added the [Glossary ID and Formality](../ai/deploy/endpoints/real-time-translation-settings.mdx#configure-translation-provider) fields to the Endpoint Real-Time Translation Settings of the DeepL Translate Pro translation provider
- Added [GPT-4 support](../ai/empower/llms/model-support-by-feature.mdx) for the LLM Prompt Node & LLM-powered Answer Extraction features
...
- Removed the [Knowledge Search](../ai/build/node-reference/other-nodes/knowledge-search.mdx) Node. Instead, the [Search Only Mode](../ai/build/node-reference/other-nodes/search-extract-output.mdx) of the Search Extract Output Node should be used

#### Bug Fixes

- Fixed the issue where the LLM test setting did not work when updating the connection with the correct API key
- Fixed the issue where the [fields in the Nodes](../ai/build/translation-and-localization/auto-translation.mdx) were not translated
...
- Added stability and new actions to the [Automation Rules](../live-agent/settings/automation-rules.mdx) section
