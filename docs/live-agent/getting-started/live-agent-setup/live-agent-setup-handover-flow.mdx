...
Once you have [Set up your Endpoint for Handover to Live Agent](live-agent-setup-handover-endpoint.mdx), you can set up your [Flow](../../../ai/build/flows/overview.mdx) for handover to Live Agent.
...
13. In the **Live Agent Settings** section, fill in the following fields:
    - **Live Agent Inbox Id** — enter the Inbox ID that you obtained in the [previous step](live-agent-setup-handover-endpoint.mdx).
...
Next, you can [add more users](live-agent-setup-agents.mdx) with `Agent` and `Administrator` roles, and assign them to Inboxes in Live Agent: