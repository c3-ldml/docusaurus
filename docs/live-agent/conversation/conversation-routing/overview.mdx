...
- [Manual](manual-mode.mdx)
- [Automatic](automatic-mode.mdx)

Conversation routing can consider a [limit per agent](../../settings/account-settings.mdx#conversation-limit-per-agent) applied to both Manual and Automatic modes and can only be set up for all Inboxes.
