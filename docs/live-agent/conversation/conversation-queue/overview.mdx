...
:::note
  ...
  The [conversation limits](../../settings/account-settings.mdx#conversation-limit-per-agent) and specific queue behavior may vary depending on the Live Agent configuration and settings in use.
...
- The Conversation limit is set to 2 conversations per agent.
- There is one online agent available.
- 3 conversations with different [priorities](../conversation-routing/automatic-mode.mdx#additional-automatic-assignment-parameters) are created:
...
- [Real-Time Queue Notifications](real-time-queue-notifications.mdx)
- [Conversation Routing: Automatic mode](../conversation-routing/automatic-mode.mdx)
- [Conversation Routing Overview](../conversation-routing/overview.mdx)