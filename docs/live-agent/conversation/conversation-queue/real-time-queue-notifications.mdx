...
When the settings are configured, you can test this configuration by creating a handover via [Demo Webchat](../../../webchat/demo.mdx).
...
If you use the [AI Copilot workspace](../../assistants/ai-copilot.mdx) in Live Agent, include additional Nodes in your AI Copilot Flow: the [Handover to Agent](#configure-settings-for-a-handover-to-agent-node) and [Lookup](#add-a-lookup-node) with the Queue status, followed by the [Say Nodes](#add-say-nodes).
In the Main Flow, below the Case Node with the Queue status,
add a [Go To Node](../../../ai/build/node-reference/logic/go-to.mdx) that leads to the AI Copilot Flow and its Lookup Node.

## More Information

- [Overview](overview.mdx)
- [Lookup](../../../ai/build/node-reference/logic/lookup.mdx)
- [User Inactivity Detection and Handling](../../../ai/escalate/user-inactivity-detection.mdx)
- [Agent Handover](../../../ai/build/node-reference/service/handover-to-agent.mdx)