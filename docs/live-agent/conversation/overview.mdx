...
- **Assigned** — a conversation is assigned to a human agent. The assignment occurs either [manually](../conversation/conversation-routing/manual-mode.mdx) or [automatically](../conversation/conversation-routing/automatic-mode.mdx).
...
| Accepted Privacy Policy | Accepted the privacy policy terms. This field is applicable to Webchat v3 when a user accepts the [Privacy Notice](../../webchat/v3/features.mdx#privacy-notice). |
...
This action will also update the [Contact Profile](../../ai/analyze/contact-profiles.mdx) on the Cognigy.AI side. In Live Agent, cached Contact Profile data will persist for one hour. You can change this parameter if you have an on-premises or dedicated SaaS environment. To do so, add the `COGNIGY_CONTACT_PROFILE_CACHE_TIME` variable in the Live Agent `values.yaml` file and specify the value in seconds.
...
For more information on these details, read the [Assign Conversations](assign-conversations.mdx) documentation.
...
- [AI Copilot Bot](../assistants/ai-copilot-bot.mdx)
- [AI Copilot Whisper](../assistants/ai-copilot-whisper.mdx)
- [AI Copilot Workspace](../assistants/ai-copilot.mdx)
...
3. On the Conversations panel, choose one of the following tabs:
    ...
    - **All** — shows all conversations in the system. All system conversations. Agents, by default, cannot view all conversations, but administrators can grant access through the **Inboxes** settings. For more information, read the [Configure Agent Access to all Inbox Conversations](../settings/inboxes.mdx#configure-agent-access-to-all-inbox-conversations).
...
- [Assign Conversations](assign-conversations.mdx)
- [Send Replies](send-reply.mdx)
- [Filter and Sort](search-and-filter.mdx)
- [Other Operations](other-operations.mdx)
