...
3. In the **Add Canned Response** window, fill in the following fields:
    ...
    - **Level** — select one of the following options from the list:
        ...
        - **Team** — only human agents from the selected [team](teams.mdx) can use a canned response. You can select only one team.
4. Click **Submit**.
...
To start using canned responses in the conversations, go to the [Send a Canned Response](../conversation/send-reply.mdx#advanced-quick-reply).