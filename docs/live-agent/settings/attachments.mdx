...
End users can send attachments to human agents by using the **Add attachments** icon in the chat if you set up a [Storage File](../../ai/deploy/endpoints/file-storage.mdx) provider in the Endpoint Settings.

## Request an Attachment with the Plugin

If you are using a [Webchat](../../webchat/overview.mdx) Endpoint with Live Agent, in addition to the standard Attach file feature, human agents can request an attachment from a user. To enable this functionality, you need to add the [File Upload plugin](https://github.com/Cognigy/WebchatPlugins/tree/master/plugins/file-upload) to the Webchat Endpoint to upload files.
...
- [Conversation Workflow](../conversation/overview.mdx)