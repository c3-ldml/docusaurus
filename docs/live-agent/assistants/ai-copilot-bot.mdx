...
- The **Main** Flow for interaction between an AI Agent and an end user. Within this Flow, add a [Handover to Agent](../../ai/build/node-reference/service/handover-to-agent.mdx) Node to transition the conversation from the AI Agent to a human agent. The Inbox ID must be included in the Handover to Agent Node.
