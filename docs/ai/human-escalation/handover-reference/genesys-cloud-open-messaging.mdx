...
The Genesys Cloud Open Messaging handover connector is based on [Open Messaging APIs](https://developer.genesys.cloud/co.mdxigital/digital/openmessaging/openmessaging-apis). [Open messaging](https://developer.genesys.cloud/co.mdxigital/digital/openmessaging/) facilitates integrations between Genesys Cloud and a third-party messaging service through a webhook.

## Prerequisites

- Access to [Genesys Cloud](https://login.mypurecloud.com/).
- Created Endpoint [compatible with Genesys Cloud Open Messaging](../../deploy/endpoints/handover-settings.mdx#endpoints-compatible-with-handover-providers).
...
- To detect handover completion in Genesys:
    - Starting from Cognigy 4.100, migrate to [Amazon EventBridge](genesys-cloud-open-messaging-with-amazon-eventbridge.mdx) using the Amazon EventBridge Genesys connector to send requests to Cognigy.AI.
    - For Cognigy 4.99 and earlier versions, you have two options:
        ...
        - As an alternative, you can configure [HTTP webhooks](genesys-cloud-open-messaging-handover-end-detection.mdx) using Triggers and Web Services Data Actions to send requests to Cognigy.AI.
...
:::info[2. Configure Handover Settings]
  ...
  In [Handover to Agent](../../build/node-reference/service/handover-to-agent.mdx) Node, configure the following settings:
...
Within the Genesys integration, you can use the [AI Copilot workspace](../../../ai-copilot/overview.mdx) as an assistant for your human agents.
...
- [Genesys Cloud Guest Chat](genesys-cloud-guest-chat.mdx)
