...
This Node is useful when paired with a [Handover to Agent](../../service/handover-to-agent.mdx) Node or a [Transfer](../generic//transfer.mdx) Node.
...
(the **Capture DTMF Signals** parameter in the [Set Session Config](set-session-config.mdx) Node) to provide feedback on the caller's input.
...
<CollapsibleSection title="Example DTMF Flow">
    ...
    Test your Flow via the Interaction Panel. Use the [Voice Call](../../../../test/interaction-panel/overview.mdx#voice-call) setting.
...
- [DTMF Parameter](parameter-details.mdx)
- [DTMF Verb](../../../../../voice-gateway/references/verbs/dtmf.mdx)
- [DTMF Collect Config](../generic//dtmf-collect-config.mdx)