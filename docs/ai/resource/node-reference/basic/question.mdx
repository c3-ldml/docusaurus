...
A Question Node is used to ask a question that requests specific information from the user. When the Node is triggered, the [Entrypoint](../../flows/editor.mdx) shifts to this Node so that the conversation continues only after the user answers. Also, a new [Input](../../../test/interaction-panel/input.mdx) object is generated.

When a user input is received, it's scored based on natural language understanding (NLU). If an attached Flow has an Intent that scores higher than Intents in the current Flow, the [attached Flow](../../../empower/nlu/attachments/attached-flows.mdx) is executed. The Intent scoring occurs before validation of the Question Node is completed.
...
<CollapsibleSection title="Question Types">
    ...
    | Intent                               | One of the trained [Intents](../../../empower/nlu/intents/ml-intents.mdx) must be identified from the user's response.                                                                                                                                                                                                                                                                                                                                                                                                   |                                                                                                                                                                        |
    | Slot                                 | A [System Slot](../../../empower/nlu/slots-and-lexicons/slots.mdx#system-defined-slots) or [Lexicon Slot](../../../empower/nlu/slots-and-lexicons/lexicons.mdx) must be detected within the user's response. The slot is defined by name.                                                                                                                                                                                                                                                                                 |                                                                                                                                                                        |
  ...
  All data formats supported by the Cognigy NLU for system slot mapping are listed on the [Slot Mapping](../../../empower/nlu/slots-and-lexicons/slots.mdx) page.
...
<CollapsibleSection title="Channels and Output Types">
    ...
    Question Node output types carry the same functionality as the [Say Node](say.mdx).
    
    :::success[Date Questions]

  If you select the Date as a Question Type, the Question Node automatically renders a datepicker if the channel supports it. Refer to [Datepicker](../data/datepicker.mdx) for more information. 
...
<CollapsibleSection title="LLM Entity Extraction Options">
    ...
    set the Generative AI provider in the [Settings](../../../empower/generative-ai.mdx#set-up-generative-ai).
...
<CollapsibleSection title="Reprompt Options">
    ...
    **Reprompt Methods**
    ...
    === "LLM Prompt"
        ...
        Before using this Node, configure a model in the [LLM](../../../empower/llms/providers/all-providers.mdx) settings.
    ...
    **Reprompt Condition**
    ...
    Optionally, you can add a [CognigyScript](../../cognigyscript.mdx) condition to determine whether a reprompt message is shown.
...
<CollapsibleSection title="Result Storage">
    ...
    If **Store Result in Context** is enabled, the Question Result is also stored in the [Context](../../../test/interaction-panel/context.mdx) object. 
    
    If **Store Result to Contact Profile** is enabled, the Question Result is also stored in the [Profile](../../../test/interaction-panel/profile.mdx) object.
...
<CollapsibleSection title="AI-Enhanced Output">
    ...
    To use AI-enhanced output rephrasing, read the [Generative AI](../../../empower/generative-ai.mdx#rephrasing-ai-agent-outputs) article.
...
<CollapsibleSection title="Answer Preprocessing">
    ...
    You can use various functions of the [Text Cleaner](../../../empower/nlu/text-cleaner.mdx) class to preprocess the answer to a question before it is evaluated. This can be helpful, for example, when requesting a name using a `text` type question or when asking for a part number using a `slot` question.
