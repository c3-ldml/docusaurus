...
<a href="../../../../../release-notes/4.68.mdx" /><img src="https://img.shields.io/badge/Updated in-v4.68-blue.svg" alt="Version badge" />
...
The `actions` object provides access to most internal Cognigy functions within the Code Node. For more information, refer to [Actions](actions.mdx).

The `getTextCleaner` function allows you to get access to an instance of the [Text Cleaner](modules.mdx).
...
- The maximum code execution time is 1 second. If the limit is exceeded, an error is returned, and the message can be accessed in the `input.codeNodeError.message` Input object. The maximum execution time isn't configurable. To run code that takes longer than one second to execute, we recommend using custom [Extensions](../../../extensions.mdx#extension-marketplace).
...
defined in the [Profile Schema](../../../../analyze/contact-profiles.mdx#profile-schema).

## Adding Log Statements to Code Nodes

To view log statements on the [Logs page](../../../../test/logs.mdx), insert `api.log()` statements into the Code Node.
...
You can send Facebook JSON directly from within Code Nodes using the [output](actions.mdx) action.
...
- [Actions](actions.mdx)
- [Input object](../../../../test/interaction-panel/input.mdx)
- [Context object](../../../../test/interaction-panel/context.mdx)
