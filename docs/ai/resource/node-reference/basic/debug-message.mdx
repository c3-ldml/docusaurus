...
<a href="../../../../release-notes/4.84.mdx" /><img src="https://img.shields.io/badge/Added in-v4.84-blue.svg" alt="Version badge" />
...
To view the debug message, enable [debug mode](../../../test/interaction-panel/overview.mdx#debug-mode). 
...
<CollapsibleSection title="Info Message">
    ...
    It can be helpful when a user fills out an [Adaptive Card](say.mdx) form or clicks the [Submit](../xApp/set-html-xApp-state.mdx) button in xApps to confirm an action.
...
<CollapsibleSection title="Error Message">
    ...
    It can be useful for tracking errors that occur during database requests. For example, it can help identify issues when using [MongoDB](../service/mongo-db.mdx) Nodes.
...
To debug a Flow when your AI Agent experiences issues on the production side, add a [Log Message](../other-nodes/log-message.mdx) Node.