...
<a href="../../../../release-notes/4.48.mdx" /><img src="https://img.shields.io/badge/Added in-v4.48.0-blue.svg" alt="Version badge" />
...
:::warning[Deprecation]
  ...
  The GPT Conversation Node has been deprecated and can no longer be created in Cognigy.AI v4.85 and later. Use the [LLM Prompt Node](llm-prompt.mdx) to generate messages with LLM services.
...
:::warning
  ...
  Before using this Node, set the Generative AI provider in the [Settings](../../../empower/generative-ai.mdx#set-up-generative-ai).
...
- **Transactional** — is focused on answering user questions based on pre-existing knowledge of the conversation topic and strict sequence of AI Agent's steps. In this mode, you can call an action, which should be handled in Flow logic after the GPT Conversation Node. You can use the [Lookup](../logic/lookup.mdx) and [If](../logic/if.mdx) Nodes to check if the context has the action and then perform a specific task accordingly, for example, create an HTTP request.
...
| Available Actions     | Text          | Actions the AI Agent can trigger. You can reuse these actions in the [Lookup](../logic/lookup.mdx) or [If](../logic/if.mdx) Nodes to check if the context has the action and then perform a specific task accordingly, for example, create an HTTP request. For more information, read [Specific Example](#specific-example). |
...
In the Transaction mode, you can use the [Lookup](../logic/lookup.mdx) or [If](../logic/if.mdx) to continue a flow logic. 
...
- [Generative AI](../../../empower/generative-ai.mdx)
- [LLM Prompt Node](llm-prompt.mdx)