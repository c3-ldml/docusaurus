...
<CollapsibleSection title="POST, PATCH, and PUT">
    ...
    **Payload**
    ...
    - **JSON** — consists of key-value pairs. When using a JSON payload to access Cognigy.AI objects such as Input, Context, or Profile, refer to [CognigyScript](../../cognigyscript.mdx#json-arguments) for an example payload.
...
<CollapsibleSection title="Storage Options">
    ...
    After the HTTP request is successfully executed, you can access the response payload by executing the following [CognigyScript](../../cognigyscript.mdx):
...
<CollapsibleSection title="Authentication">
    ...
    Authentication makes use of [Connections](../../connections.mdx), which means that the actual authentication information can be encrypted. 
...
<CollapsibleSection title="Error Handling & Logging">
    ...
    **Debug Mode Logging**
    ...
    to log to [Debug Mode](../../../test/interaction-panel/overview.mdx#debug-mode) when using the Interaction Panel:
