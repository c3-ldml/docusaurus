...
<a href="../../release-notes/4.95.mdx" /><img src="https://img.shields.io/badge/Updated in-v4.98-blue.svg" alt="Version badge" />

_Tokens_ are visual representations of [CognigyScript](cognigyscript.mdx) expressions that you store and reuse in Cognigy.AI. Cognigy.AI has a selection of default Tokens that are associated with user Input, Context, and Profile objects.
...
- **Type** — the category of the Token in the search list:
    - **Input** — access to the data in the [Input](../test/interaction-panel/input.mdx) object.
    - **Context** — access to the data in the [Context](../test/interaction-panel/context.mdx) object.
    - **Profile** — access to the data in the [Profile](../test/interaction-panel/profile.mdx) object.
    - **Custom** — specific CognigyScript expression with access to the data in one or more [Cognigy objects](cognigyscript.mdx#cognigy-objects). For example, `"Should your order " + context.customerOrderAmount + " be sent to " + profile.address + "?"`.
...
<CollapsibleSection title="Default Tokens">
    ...
    | Short-Term Memory            | Context    | Displays the short-term memory of the conversation, as defined in the [AI Agent Node settings](node-reference/ai/ai-agent.mdx).                                                                                                                                                        | `context.shortTermMemory`                                         |
