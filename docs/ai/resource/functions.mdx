...
The execution of Functions is independent of Flows, but you can trigger them from a Flow. You can also use the [Inject & Notify](../deploy/endpoints/inject-and-notify.mdx) APIs in a Function to send results back into a Flow.
...
<Tabs>
  <TabItem value="tab1" label="GUI" default>
    ...
    Also, you can trigger a Function from the Flow editor by using the [Trigger Function Node](node-reference/service/trigger-function.mdx).
...
<CollapsibleSection title="Interact with Flows">
    ...
    Functions can interact with Flows through the [Inject & Notify](../deploy/endpoints/inject-and-notify.mdx) APIs.
