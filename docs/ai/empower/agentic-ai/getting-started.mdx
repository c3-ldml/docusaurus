...
<a href="../../../release-notes/4.91.mdx" /><img src="https://img.shields.io/badge/Added in-v4.91-blue.svg" alt="Version badge" />
...
- Ensure you have access to your Cognigy.AI account and have [configured a primary LLM](overview.mdx#prerequisites).
- During the Flow Configuration steps, an [Email Notification Node](../../build/node-reference/data/email-notification.mdx) will be used. Make sure your SMTP server settings are properly configured to receive notifications.
...
1. Below the parent AI Agent Node, configure the [child Tool Action Node](../../build/node-reference/ai/ai-agent.mdx#ai-agent-tool-settings). Left-click the **unlock_account** Node to open the Node editor.
...
3. Below the child Tool Node, replace the placeholder Node with an [Email Notification](../../build/node-reference/data/email-notification.mdx) Node. The Email Notification Node acts as an Action Node, designed to trigger specific actions and perform the task assigned to the tool. In this case, the Email Notification Node will send a confirmation email when the task is completed.
...
5. Below the Action Node, add a [Resolve Tool Action](../../build/node-reference/ai/resolve-tool-action.mdx) Node to control how the AI Agent responds after executing the Email Notification Node. The Resolve Tool Action Node returns the conversation to the parent AI Agent Node, providing the option to choose a different tool.
...
1. Try [creating an AI Agent tailored to your business needs](manage-ai-agents.mdx#create-an-ai-agent) or [hire one from the Job Market](manage-ai-agents.mdx#create-an-ai-agent).
2. [Add the AI Agent to your Flow](../../build/flows/overview.mdx) and [create a set of tool actions](../../build/node-reference/ai/ai-agent.mdx) for your AI Agent to operate.
