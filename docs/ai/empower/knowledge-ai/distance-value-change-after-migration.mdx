...
These differences lead to a new interpretation of the `distance` value. If you use `distance` to make decisions in the [Flow](../../build/flows/overview.mdx), consider the new similarity range of -1 to 1.

:::warning

  The [LLM embedding model](../llms/overview.mdx) you use defines how precisely Knowledge AI calculates `distance`. The different LLM embedding models can provide considerably different `distance` values. Use the `distance` value for decision-making in Flows with caution.
