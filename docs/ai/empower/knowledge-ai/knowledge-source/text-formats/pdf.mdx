...
The Default Chunk Strategy extracts content from PDF and splits it into [Knowledge Chunks](../../knowledge-chunk/knowledge-chunk.mdx) based on a fixed token length and recognizes tables. Based on our research, the Default Chunk Strategy returns better structured results and works best with the **Top K** value set to 5 in the [Search Extract Output](../../../../build/node-reference/other-nodes/search-extract-output.mdx) Node.
