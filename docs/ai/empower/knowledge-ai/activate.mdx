...
A user with `admin` rights needs to add the license to your installation. If you are using a shared SaaS installation with version 4.78 or later, contact [Cognigy technical support](../../../help/get-help.mdx) to add a quota for your organization. For on-premises and dedicated SaaS installations, contact Customer Success Management or Account Executives at Cognigy.
...
Only a user with `admin` rights for the Management UI can set quotas via the Management UI or the Cognigy.AI API. If you use a shared SaaS environment, contact [Cognigy technical support](../../../help/get-help.mdx) to configure quotas. For on-premises and dedicated SaaS installations, contact Customer Success Management or Account Executives at Cognigy.
...
- **Number of Knowledge Queries** — Knowledge AI counts the number of Queries performed to retrieve relevant Knowledge Chunks. The number of Knowledge Queries is unlimited. However, you will be billed based on the Queries you have performed. Each time the following Nodes are executed they increase the number of performed Knowledge Queries by one:
    - [AI Agent Node](../../build/node-reference/ai/ai-agent.mdx)
...
To activate the Knowledge AI feature flag, contact [Cognigy technical support](../../../help/get-help.mdx).

## View Metrics

Familiarize yourself with the [Admin Center](../../administer/access/admin-center/overview.mdx) page. On this page, you can monitor Knowledge AI metrics to manage the consumption of Knowledge AI resources effectively.
