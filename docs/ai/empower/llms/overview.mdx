...
:::info[Automate Routine Tasks]
  ...
  - **Generate Flows**. LLMs can generate Flows based on the provided title and description, helping save time during conversation design iterations. To generate Flows, refer to the [Generate Flows](../generative-ai.mdx#generate-flows) section.
  - **Generate Lexicons**. With LLMs, you can build a list of Keyphrases for the language of your choice based on the title and description of the provided Lexicon. To generate Lexicons, refer to the [Generate Lexicons](../generative-ai.mdx#generate-lexicons) section.
  - **Generate Intent sentences**. With LLMs, you can build a list of Intent sentences based on the title and description of the provided Intent. To generate Intents, refer to the [Generate Intent Sentences](../generative-ai.mdx#generate-intent-sentences) section.
  - **Generate adaptive cards**. LLMs can create Adaptive Cards based on the prompt you provide in the description field for the **Adaptive Card** output type in the [Say](../../build/node-reference/basic/say.mdx#adaptive-card) or [Copilot: Adaptive Card Tile](../../build/node-reference/ai-copilot/set-adaptive-card-tile.mdx) Nodes.
...
:::info[Improve AI Agent's Output]
  ...
  - **Generate Enhanced Output**. By using LLMs, AI Agents can produce responses that are more comprehensive, nuanced, and in line with customer needs. This enhancement is important in handling queries that require a level of empathy or complex information processing. To generate enhanced output, add [LLM Prompt Nodes](../../build/node-reference/service/llm-prompt.mdx) within your Flow.
  - **Rephrase AI Agent's Output**. LLMs are good at rephrasing texts while maintaining the original meaning. This capability allows for smoother communication if the original output of an AI Agent lacks clarity or tone alignment. To use this capability, enable AI-enhanced rephrasing in [Say](../../build/node-reference/basic/say.mdx#ai-enhanced-output), [Question](../../build/node-reference/basic/question.mdx), or [Optional Question](../../build/node-reference/basic/optional-question.mdx) Nodes by selecting the **Rephrase Output** option in the **AI-enhanced output** section. Also, you can generate dynamic reprompts for incorrect answers via [Question Nodes](../../build/node-reference/basic/question.mdx#reprompt-methods).
  - **Train Intents and Find the Right Intent**. To enhance AI Agents' understanding of user inputs in various languages, use an [external embedding LLM](../nlu/external/external-nlu-intent-recognition.mdx). Also, optimize intent classification accuracy by using [LLM-based intent reranking](../nlu/external/intent-reranking.mdx), which prioritizes and selects the most relevant intent.
...
:::info[Extract Specific Entities from User Inputs]
  ...
  add the [LLM Entity Extract](../../build/node-reference/other-nodes/llm-entity-extract.mdx) Node within your Flow.
...
:::info[Determine the Emotional Tone of the Conversation]
  ...
  using an [AI Copilot: Sentiment Analysis Tile](../../build/node-reference/ai-copilot/sentiment-tile.mdx) Node. The widget monitors the conversation and displays the emotional tone (positive, negative, or neutral) of the latest user input in real time.
...
:::info[Ensure LLM Accuracy with Human Review]
  ...
  AI Copilot provides [plug-and-play widgets](../../../ai-copilot/plug-and-play-widgets.mdx) in the [workspace](../../../ai-copilot/index.mdx), allowing human agents to review the output generated by LLMs before sending responses to customers. 
...
To use a standard model, select an LLM provider and model name from the list of [predefined models](model-support-by-feature.mdx). 
By choosing a standard model, you can ensure that this model is fully compatible with Cognigy.AI features.

After selecting a model, you must enter the API credentials from the [LLM provider](providers/all-providers.mdx) that offers this model to get access to its functionality.
...
Similar to the standard model, you must enter the API credentials from the [LLM provider](providers/all-providers.mdx) that offers this model to get access to its functionality.
...
- [LLM Prompt](../../build/node-reference/service/llm-prompt.mdx)
- [LLM Entity Extract](../../build/node-reference/other-nodes/llm-entity-extract.mdx)
- [GPT Conversation](../../build/node-reference/service/gpt-conversation.mdx)
- [Search Extract Output](../../build/node-reference/other-nodes/search-extract-output.mdx)
- Question Node Slot Mapping (in case of using [external NLU embedding](../nlu/external/external-nlu-intent-recognition.mdx))
- [Question Node LLM Reprompting](../../build/node-reference/basic/question.mdx#llm-prompt)
- [Sentiment Analysis](../../build/node-reference/ai-copilot/sentiment-tile.mdx)
...
As an alternative to the [default retry mechanism](#retry-mechanism) that is applicable to all models, you can override it for a specific model by using an [LLM fallback](fallback.mdx).
...
- [Supported LLM Providers](providers/all-providers.mdx)
...
- [Knowledge AI](../knowledge-ai/overview.mdx)