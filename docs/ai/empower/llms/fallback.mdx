...
<a href="../../../release-notes/4.90.mdx" /><img src="https://img.shields.io/badge/Added in-v4.90(Beta)-purple.svg" alt="Version badge" />
...
minimizing disruptions for AI Agents during [design](../generative-ai.mdx#design-time-generative-ai-features) and [run-time](../generative-ai.mdx#run-time-generative-ai-features) operations.
...
- Using a fallback model overrides the [default retry mechanism](overview.mdx#retry-mechanism).
- You can add only one fallback to a main model.

## Create a Fallback Model

Creating a fallback model works the same way as creating a main model.
If your Project has only one model, you can add another by choosing one [from the same LLM provider or another provider](providers/all-providers.mdx) with similar capabilities.
