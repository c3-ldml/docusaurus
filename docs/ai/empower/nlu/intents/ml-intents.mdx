...
A new [Task](../../../build/projects.mdx) will begin, and the spinning task icon indicates model building is in progress. While new models are built, the latest version of your flow will fall back to the most recently trained model.
...
When clicking the arrow on the **Build Model** button, a **Quick Build** option can be selected. This option will build a machine learning model for your intents which doesn't contain specifications for [States](../../../test/interaction-panel/state.mdx) and Intent Conditions and is thus significantly faster in building your model.

<img src={require('../../../../../static/img/_assets/ai/empower/nlu/quick-build.JPG').default} width="100%" />

If you're not using [States](../../../test/interaction-panel/state.mdx) or Intent Conditions, the training speed (as in the time it takes for Cognigy.AI to build the model after you clicked the button) and quality of the model will be the same whether you use the regular *Build Model* or *Quick Build* options.
...
:::warning[Avoid Repetition]
  ...
  Rather than writing similar sentences for different entity names i.e. `I want to order Pizza, I want to order Pasta...` build and attach a Lexicon to the flow that can be [Annotated](annotations.mdx) into your sentence.
...
:::note[Evaluation of Example Sentences]
  ...
  Find out more about how Cognigy.AI evaluates example sentences on the [Intent Analyzer Page](intent-analyzer.mdx).
...
To use Generative AI technology for creating Intent example sentences, read the [Generative AI](../../../empower/generative-ai.mdx#generate-intent-sentences) article.
...
:::note[Example Sentence Annotation]
  ...
  Find out more about recognizing **Slots** and **Lexicons** in example sentences on the [Annotations](annotations.mdx) page.
...
You can change these thresholds in [Agent Settings](../../../administer/access/project-settings.mdx).
