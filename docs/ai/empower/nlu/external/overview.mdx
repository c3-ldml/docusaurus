...
Cognigy.AI is set up to handle all intent and slot mapping for agents with the built-in Cognigy NLU using the resources defined in flow [Intents](../intents/ml-intents.mdx) and [Lexicons](../slots-and-lexicons/lexicons.mdx). 

However, it is also possible to integrate an external NLU Provider to handle intent mapping within Cognigy.AI using the [NLU Connectors](nlu-connectors.mdx) feature. These external NLU Connectors will generate the same input object as Cognigy, which enables agents to easily switch NLU providers without having to change flows. 
...
To create a new NLU Connection, open an [Agent](../../../build/projects.mdx) and navigate to the [NLU Connectors](nlu-connectors.mdx) menu available from the menu on the left side of the interface under the **Build** section. 
...
---
An external NLU service that has been linked via an [NLU Connectors](nlu-connectors.mdx) feature can be used to deploy to any [Endpoint](../../../deploy/endpoints/overview.mdx) that communicates with a deployment channel. This allows different conversational channels to use different NLU's when connected to the same agent in Cognigy.AI.
