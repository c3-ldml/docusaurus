...
import forbiddenCharacters from '@site/docs/_includes/ai/_forbidden-characters.mdx';
...
If the NLU predicts the utterance is out of scope of the intent model, no intent will be triggered and intent will be set to `null` in the [Input](../../test/interaction-panel/input.mdx) object. You can add examples to the [Reject Intent](intents/reject-intents.mdx) to intentionally prevent the NLU from recognizing any user inputs that are outside the scope of the AI Agent.
...
### Default Replies
Intents can be configured with direct output (answers) in the form of so-called **Default Replies**. Default Replies are integrated [Say](../../build/node-reference/basic/say.mdx) and can be configured with channel-specific output.

<img src={require('../../../../static/img/_assets/ai/empower/nlu/intent_settings_default_replies_training.png').default} width="100%" />

You can override the setting to use the **Default Replies** as example sentences per each individual Intent. Per default, the setting is set to **Use Flow [Settings](../../administer/access/project-settings.mdx)**.
...
See [Intent Conditions](intents/conditions.mdx) for more information on how to enable and disable Intents dynamically with CognigyScript Conditions.
...
To learn how to add reconfirmation sentences, read [Machine Learning Intents](intents/ml-intents.mdx).
...
* **Create Child Intent**. Adds an intent as a child intent to the selection. See [Intent Hierarchy](intents/intent-hierarchy.mdx). This property is included on data import/export.
...
For more information, read the [State](../../test/interaction-panel/state.mdx) article.
...
For more information, read the [Slot Fillers](slot-fillers.mdx) article.
