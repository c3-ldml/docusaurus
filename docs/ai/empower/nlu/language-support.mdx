...
- **Shared SaaS**. On `trial`, `app`, and `au-02`, you already have access to a preinstalled list of supported languages. However, if you're on `app-us` and `app-uk` and want to add a language other than English or German, contact [Cognigy technical support](../../../help/get-help.mdx).
- **Dedicated SaaS**. To extend the initial list of supported languages, contact [Cognigy technical support](../../../help/get-help.mdx).
- **On-premises**. To add languages to your environment, refer to the installation [guide](../../installation/migration/from-nlu-v1-to-v2-migration.mdx#choose-which-languages-to-deploy).
...
Which means that as soon as the user mentions the word pizza, this Intent will be triggered. For more information on how to use these Intents, go [here](intents/rule-intents.mdx).
