...
Flow functionality is shared between Flows by making their [Intents](../overview.mdx#intents) available in the other Flow. If the Intent of the attached Flow is scored highest, the attached Flow is executed instead of the main Flow.
...
:::warning[Beware of Overlapping Intents]
  ...
  The order in which the Intents are scored is determined by the `Map global Intents first` setting in [Agent Settings](../../../administer/access/project-settings.mdx).
