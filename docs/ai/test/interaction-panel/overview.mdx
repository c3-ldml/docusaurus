...
import VoiceProviders from '@site/docs/_includes/ai/_voice-providers.mdx';
...
[Locales](../../resource/translation-and-localization/localization.mdx) are fundamental components for multilingual AI Agents communication.
...
When you run a Flow on the **Test** tab, the AI Agent's responses will match the language you previously selected in the [locale settings](../../resource/flows/overview.mdx) of your Flow.

### Snapshot

[Snapshots](../../deploy/snapshots.mdx) are immutable forms of your AI Agent.
...
For more information, refer to [Snapshots](../../deploy/snapshots.mdx).
...
To view channel-specific output, it must be configured in an output Node (for example, [Say](../../resource/node-reference/basic/say.mdx) or [Question](../../resource/node-reference/basic/question.mdx) Nodes).
...
Messages can include attached data. You can simulate this data input by adding a data payload in [JSON](https://www.json.org/json-en.html) format to the data input field. To activate the **Data Input** field, go to the **Settings** tab and enable the **Show data input** toggle. Any message data sent to Cognigy.AI is accessible via [CognigyScript](../../resource/cognigyscript.mdx) using the expression `{{ " {{input.data}}" }}`.

In **Chat** mode, you can use additional settings:

- **Reset Session** — the setting clears the chat history in the Interaction Panel. 
- **Create Playbook** — the setting starts a new Playbook for testing conversations. For more information, refer to [Playbooks](../playbooks.mdx).
- **Create Playbook with Assertions** — the setting starts a new Playbook with assertions for in-depth testing. For more information, refer to [Playbook with Assertions](../playbooks.mdx#assertion).
- **Enable/Disable Debug Mode** — the setting toggles the [Debug mode](#debug-mode). Debug mode is enabled by default.

#### Voice Call

<a href="../../../release-notes/earlier-versions/overview.mdx" /><img src="https://img.shields.io/badge/Beta-purple.svg" alt="Version badge" />
...
:::note[Feature availability]
  ...
  Before starting a voice call, you need to configure a voice provider in the [Voice Preview Settings](../../test/voice-preview.mdx) of your Project. 
...
For more information, refer to [Assertions](../../test/playbooks.mdx#assertion).
...
refer to [Cognigy Objects Life Span](../../resource/cognigyscript.mdx#life-span-of-cognigy-objects).
...
For more information, refer to [Input](input.mdx).
...
For more information, refer to [Context](context.mdx).
...
For more information, refer to [Profile](profile.mdx).
...
For more information, refer to [State](state.mdx).
...
For more information on how to manage NLU connectors, refer to [NLU Connectors](../../empower/nlu/external/nlu-connectors.mdx).
...
- Triggered [Intents](../../empower/nlu/intents/ml-intents.mdx) and their [scores](../../empower/nlu/intents/intent-analyzer.mdx).
- Triggered [Yes/No Intents](../../empower/nlu/intents/yes-no-intents.mdx) and their [scores](../../empower/nlu/intents/intent-analyzer.mdx).
- Matched [Slots](../../empower/nlu/slots-and-lexicons/slots.mdx)
- Found [Lexicon Keyphrases](../../empower/nlu/slots-and-lexicons/lexicons.mdx)
- [State](./state.mdx) changes
- Flow Transitions, for example, when executing a [Go To Node](../../resource/node-reference/logic/go-to.mdx), an [Execute Flow Node](../../resource/node-reference/logic/execute-flow.mdx), or triggering an [Attached Flow](../../empower/nlu/attachments/attached-flows.mdx).
- Node-specific output, for example, from an [HTTP Request Node](../../resource/node-reference/service/http-request.mdx). Some Nodes have settings to further configure their Debug mode output. Node-specific outputs can be disabled selectively in the Interaction Panel [settings](#settings).

Debug Messages can also be triggered explicitly, either using the [Debug Message](../../resource/node-reference/basic/debug-message.mdx) Node or [debug actions](../../resource/node-reference/basic/code/actions.mdx) in the Code Node.
...
<a href="../../../release-notes/index.mdx" /><img src="https://img.shields.io/badge/Beta-purple.svg" alt="Version badge" />
...
- [Cognigy Script and Objects Life Span](../../resource/cognigyscript.mdx#life-span-of-cognigy-objects)
...
- [Voice Gateway Parameter Details](../../resource/node-reference/voice/voice-gateway/parameter-details.mdx)