...
This data is accessed by connecting to the [OData Analytics Endpoint](odata.mdx). 
...
using the [Overwrite Analytics](../build/node-reference/analytics/overwrite-analytics.mdx) or [Code](../build/node-reference/basic/code/overview.mdx) Nodes.
...
2. While a Flow is processing the Input, the Flow can overwrite the initialized data. This can be done by using the [Overwrite Analytics Node](../build/node-reference/analytics/overwrite-analytics.mdx) or [Code Node](../build/node-reference/basic/code/overview.mdx). 
