...
import understoodMessages from '@site/docs/_includes/ai/_understood-messages.mdx';
...
<Video
  src="https://www.youtube.com/embed/D18JNjUPvN4"
  title="Cognigy Sessions: Cognigy Insights & OData"
  startTime={1857}
  width="70%"
/>

To analyze your AI Agent's performance, you can use the [built-in Insights reports](../../insights/overview.mdx) as an alternative to OData or in addition to it.
...
You can control data available through the OData endpoint at both the [Endpoint](../deploy/endpoints/overview.mdx) and [Flow](../build/flows/overview.mdx) levels:

- **Endpoint**. Go to the [Data Protection & Analytics](../deploy/endpoints/data-protection-and-analytics.mdx) section within the Endpoint, which manages data associated with your connections to channels such as webchat, Slack, and WhatsApp.
...
- **Node**. Add a [Blind Mode](../build/node-reference/analytics/blind-mode.mdx) Node to your Flow. This Node disables or masks analytics data available in the OData Endpoint.
...
To connect to the OData endpoint, you need to have the `odata` [global role](../../ai/administer/access/admin-center/access-control.mdx#global-roles)
and an [API Key](../../ai/administer/user-menu/my-profile.mdx#create-api-keys).
...
- `your-api-key` — an API key that provides authentication and authorization for accessing the API. You need to get this key from your [profile](../../ai/administer/user-menu/my-profile.mdx#create-api-keys).
...
Each collection can be filtered by its fields, which are often retrieved directly from the [Input](../test/interaction-panel/input.mdx) object results.
...
by using the [Overwrite Analytics Node](../build/node-reference/analytics/overwrite-analytics.mdx).
...
| mode               | A [Mode](../test/interaction-panel/input.mdx#properties) of the input.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | String   | TextOnly                                                         |
...
by setting a value in the [Analytics Step](../analyze/collecting-data.mdx#analytics-data-flow) field.
...
The LiveAgentEscalations collection contains records from [Cognigy Live Agent](../../live-agent/overview.mdx) and includes the related data about handovers to Live Agent.
...
The Goals collection contains records from [Goals](goals-and-tasks/overview.mdx).
...
The GoalSteps collection contains records about a specific goal step from [Goals](goals-and-tasks/overview.mdx).
Each goal is logged as a single record.
...
The GoalStepMetrics collection contains records about specific goal-related events from [Goals](goals-and-tasks/overview.mdx).
...
The GoalEvents collection contains records about specific goal-related events from [Goals](goals-and-tasks/overview.mdx).
...
| projectId         | The ID of the Project related to the record.                                                                                                                                                               | String     | 5a91d194fde28b0011ce2422                                         |
| organisationId    | The ID of the Cognigy.AI organization associated with the record.                                                                                                                                          | String     | 5eb4a49d426cd3d05f2892a9                                         |
| expiresAt         | The [expiration date](../administer/access/management-ui.mdx) for the goal record. After that time, the goal record will be removed from the Analytics database. | Timestamp  | null                                                             |
| localeName        | The name of the locale.                                                                                                                                                                                    | String     | English                                                          |
| endpointName      | The name of the Cognigy.AI Endpoint.                                                                                                                                                                       | String     | LA Chatbot                                                       |
| endpointType      | The type of the Endpoint.                                                                                                                                                                                  | String     | webchat2                                                         |
| endpointUrlToken  | The URL token for the Endpoint.                                                                                                                                                                            | String     | ea30b8f20db52f9d86ea36fd55a7d66bd2c4c60eb24ac7ad52f1c9e173dd4cdb |
...
- [Collecting Data](collecting-data.mdx)
- [Insights](insights.mdx)