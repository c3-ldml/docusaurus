...
:::warning[Removal of the AudioCodes Endpoint]
  ...
  The AudioCodes Endpoint was deprecated in April 2025 and will be removed in October 2025. Migrate to the [Voice Gateway Endpoint](voice-gateway.mdx) before the removal date.
...
Cognigy.AI comes with a large number of inbuilt Nodes to control the AudioCodes behaviour. See [AudioCodes Nodes](../../resource/node-reference/voice/audiocodes/overview.mdx) for more information.

## Number Metadata

AudioCodes identifies information about the caller and adds it to the Cognigy [Input](../../test/interaction-panel/input.mdx) Object as `input.data.numberMetaData`.
...
:::note[NumberMetaData in Tokens]
  ...
  All of the above are available as [Tokens](../../resource/tokens.mdx) inside Cognigy Text fields as well.
