...
The `Execution Finished Transformer` is triggered when the Flow execution has finished. However, the behavior of the Transformer function differs greatly depending on which [base type](transformers.mdx#different-base-transformer-types) of Transformer is being used, which is described in more detail below.
