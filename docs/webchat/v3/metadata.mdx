...
This data is stored in the [Input](../../ai/test/interaction-panel/input.mdx) object, enabling you to create more tailored and effective interactions with the user.
After the session, the data is available through the [Insights Transcript Explorer](../../insights/explorers/transcript.mdx) and [Cognigy.AI OData endpoint](../../ai/analyze/odata.mdx).
...
You can test the metadata object only in [Live Follow](../../ai/test/interaction-panel/overview.mdx#live-follow) mode.
...
5. Get and copy the [user ID](../../ai/analyze/user-and-session-identification.mdx#user-id) from the chat interaction. Use developer tools of your browser. For example, in the Google Chrome browser, open **Developer Tools**, navigate to **Application > Storage**, and expand **Session Storage**. Click a domain to view its key-value pairs. Next to the `userId` key, copy the value.
6. In the [Interaction Panel](../../ai/test/interaction-panel/overview.mdx), paste the user ID to the **User ID** field on the Live Follow tab.
7. Send a message as a user via the [Demo Webchat](../demo.mdx) interface. Once the message is received, you will see the same message on the **Live Follow** tab.
...
- [Endpoint Configuration](configuration.mdx)
- [Interaction Panel](../../ai/test/interaction-panel/overview.mdx)
- [Input Object](../../ai/test/interaction-panel/input.mdx)