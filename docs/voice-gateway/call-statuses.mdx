...
In Cognigy.AI, you can monitor [call events in your Flow](references/events/overview.mdx) to track the progress of both inbound and outbound calls. 
...
To receive webhook responses for outbound calls, you need to add the [`notifyUrl`](creating-outbound-calls.mdx#create-an-outbound-call-via-api-request) parameter with the appropriate webhook URL to the outbound call request. 
...
- [Create Outbound Calls](creating-outbound-calls.mdx)
