...
The [Call Recording](../../webapp/recent-calls.mdx#call-recordings) feature relies on this verb.
...
The `listen` verb can also be nested in a [`dial`](dial.mdx) or [`config`](config.mdx) verb, allowing the audio for a call between two parties to be sent to a remote WebSocket server.
...
| transcribe                    | A nested [transcribe](transcribe.mdx) verb.                                                                                                                                                                                                                                                                                                                            | No       |
...
:::warning
  ...
  Bidirectional audio is not supported when the [`listen`](listen.mdx) is nested in the context of a [`dial`](dial.mdx) verb.
