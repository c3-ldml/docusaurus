...
that allow the [Endpoint](../getting-started.mdx#create-a-voice-gateway-endpoint) to be routed to Voice Gateway.
...
an admin can listen to and review recordings on the [Recent calls](recent-calls.mdx#call-recordings) page.

Note that this feature records the entire call and cannot be managed within a Cognigy.AI Flow.
If you want to control call recording in your Flow, use a [Call Recording](../../ai/build/node-reference/voice/voice-gateway/call-recording.mdx) Node.
...
9. Scroll down to the **Call recording configuration** section, activate the **Record all calls** setting. This setting is hidden if you've enabled **Record all calls for this account** at the [account level](accounts.mdx).
10. Click **Save**.

All calls from this application will be recorded and appear in the [Recent calls](recent-calls.mdx#call-recordings) page.
