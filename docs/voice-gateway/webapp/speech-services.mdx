...
For the list of supported vendors and their Speech-To-Text and Text-To-Speech capabilities, see the [TTS and STT Vendors](../references/tts-and-stt-vendors.mdx) reference.
...
Once you created a speech service, add this service to the [Application](./applications.mdx).
...
6. In the **Label** field, create a label only if you need to create multiple speech services from the same vendor. Then, use the label in your [application](applications.mdx#add-additional-tts-and-stt-vendor) to specify which service to use.
...
such as [Set Session Config](../../ai/build/node-reference/voice/voice-gateway/set-session-config.mdx),
[Say](../../ai/build/node-reference/basic/say.mdx), [Question](../../ai/build/node-reference/basic/question.mdx) or [Optional Question](../../ai/build/node-reference/basic/optional-question.mdx),
or [Session Speech Parameters Config](../../ai/build/node-reference/voice/generic/session-speech-parameters-config.mdx).

## More Information

- [TTS and STT Vendors](../references/tts-and-stt-vendors.mdx)