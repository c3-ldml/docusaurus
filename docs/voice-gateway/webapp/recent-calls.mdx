...
*Recent Calls* are calls of your [account](accounts.mdx). Voice Gateway provides general data about the calls, such as the Date, Direction, From, To, SIP Trunk, and Call Duration. 
...
| trunk                | A [carrier](carriers.mdx) name.                                                                                                          |
...
To enable call recording, use the [Account](accounts.mdx#call-recording-configuration) or [Application](applications.mdx#call-recording-configuration) settings.

When call recording is enabled, you can use the embedded media player to listen to recordings of calls. Note that the recording will consist of only the caller track and the agent track. The dub track for [Atmosphere Sounds](../../ai/build/node-reference/voice/voice-gateway/parameter-details.mdx) will not be included in the recording.
