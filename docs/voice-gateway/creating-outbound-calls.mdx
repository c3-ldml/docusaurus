...
Before initiating an outbound call, ensure you configure an [application](webapp/applications.mdx), and the phone number you intend to call from is added to [Phone Numbers](webapp/phone-numbers.mdx).

To initiate an outgoing call, use one of the following methods:

- [Transfer Node](../ai/build/node-reference/voice/voice-gateway/transfer.mdx)
- REST API request:
    -  [HTTP Request Node](../ai/build/node-reference/service/http-request.mdx)
...
This token is filled with an [API key value](webapp/accounts.mdx#account-level-api-keys) generated in the Voice Gateway Self-Service Portal.
...
| to                        | [TargetType](references/verbs/dial.mdx#target-types) | The destination target, either a phone number or a SIP URI.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Yes      |
...
| .mdx                       | Object ([.mdx](references/verbs/.mdx.mdx))             | The Automatic Machine Detection (.mdx) feature, which identifies whether a human or a machine answered a call.                                                                                                                                                                                                                                                                                                                                                                                                                              | No       |
...
we enable the [Answering Machine Detection](references/verbs/.mdx.mdx) feature
using the `.mdx` parameter
and use the `actionHook` parameter to send the events to the [Webhook](https://webhook.site/) site.
Via `timeout`, we define a maximum ringing time. If the call is not answered within this time, Voice Gateway stops calling (see the [NO_ANSWER](references/events/NO_ANSWER.mdx) event).
...
To implement this use case, use a [Transfer Node](../ai/build/node-reference/voice/voice-gateway/transfer.mdx) in your voice Flow.
...
To implement this use case, use an [HTTP Request Node](../ai/build/node-reference/service/http-request.mdx) in your voice Flow, or create an [API Request via Postman or CLI](#create-an-outbound-call-via-api-request).

## More Information

- [Outbound Call Statuses](call-statuses.mdx)
- [Outbound Call Routing](webapp/outbound-call-routing.mdx)
